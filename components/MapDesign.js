...
const MapDesign = (props) => {
    ...
    if (!mapData) mapData = { static: {cells: []} }
    ...
    const[a, setA] = useState(["tree", 1])
    if(!mapData.static.cells) mapData.static.cells = []
    
    

    if(newCells.length == 0) for(const cellId in mapData.static.cells) newCells[cellId] = {l: mapData.static.cells[cellId].l, c: mapData.static.cells[cellId].c}
    ...
    const FightStartingMessage = payload => {
        fillGrid()
    ...
    function drawStatic(source, cellId, t) {
        ...
        if (source == "background") {
            ...
            image.src = src.src;
            console.log(staticRef.current.width)
        ...
        const src = require(`../public/${assets[source[0]][source[1]]}`).default
        const image = new window.Image();
        image.src = src.src;
        const ratio = src.width / src.height
        const imgWidth = src.width < width ? src.width : (source[0] == "tree" ? width * 2 : width)
        const imgHeight = imgWidth / ratio
        console.log(newCells[cellId])
        newCells[cellId][source[0].charAt(0)] = source[1];
        console.log(cellId, source[0].charAt(0),source[1], newCells)
        return new Promise(r => {
    ...
    const cellClicked = async (ev, direct = false) => {
        ...
        return drawStatic(a, index, staticRef.current.getContext('2d'));
    ...
    const cellHovered = async (ev, direct = false) => {
        ...
        if ((index || index == 0) && isWalkable(mapData.static.cells[index])) {
            ...
            if (socket.player.isFighting && socket.player.ourTurn) {
    ...
    const fillGrid = (highlights = { cells: [] }) => {
        ...
        for (const cellId in mapData.static.cells) {
            ...
            // console.log(highlights.cells, cellId)
    ...
    const fillStatic = async (t) => {
        ...
        //     if (!isWalkable(mapData.static.cells[cellId])) {
        //         // t.fillStyle = "green"
        //         // t.strokeStyle = "green"
        //         // t.fill()
        //         // t.stroke()
        //         /*Static sprites*/
        //         await drawStatic(!mapData.static.cells[cellId].l ? "tree" : "grass", cellId, t)
        //     } else {
        //         // t.fillStyle = "#fff"
        //         // t.strokeStyle = "#fff"
        //         // t.fill()
    ...
    useEffect(() => {
        ...
        for (const monsterGroup of mapData.monsters) {
    ...
    return (
        <div className={styles.mapContainer} ref={containerRef}>
            <div style={{position: "absolute", left: -150, display: "flex", flexDirection: "column"}}>
                {
                    assets.tree.map((e,i) => 
                        (<button onClick = {()=>setA(["tree", i])}>{`tree${i+1}`}</button>)
                    ).concat( assets.rock.map((e,i) => 
                        (<button onClick = {()=>setA(["rock", i])}>{`rock${i+1}`}</button>)
                    )).concat( assets.grass.map((e,i) => 
                        (<button onClick = {()=>setA(["grass", i])}>{`grass  ${i+1}`}</button>)
                    ))
